...
The target metadata is ``Base.metadata`` (all ORM models), and the DB URL comes from the
application's ``EnvSettings`` so migrations use the same connection as the app. Autogenerate
...
config.set_main_option("sqlalchemy.url", get_env().postgres_dsn)
